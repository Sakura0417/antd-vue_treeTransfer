...
export function useTreeTransfer(props: Props): UseTreeTransfer {
  ...
  function onChange(targetKeys: string[], direction: string) {
    ...
    if (direction === 'right') {
      ...
      console.log('-----右侧更改之前--------')
      console.log('state.rightTreeData', state.rightTreeData)
      console.log('state.leftTreeData', state.leftTreeData)
      console.log('state.leftCheckedKey', state.leftCheckedKey)
      console.log('state.targetKeys', state.targetKeys)
      console.log('leftHalfCheckedKeys', state.leftHalfCheckedKeys)
      console.log('rightCheckedKey', state.rightCheckedKey)
      console.log('--------------------')
      ...
      debugger
      state.leftTreeData.map((item) => {
        if (item.children) {
          if (item.children?.every((i) => i.disabled)) {
            item.disabled = true
          } else {
            item.disabled = false
            console.log('item=============>', item.id)
            debugger
            state.leftCheckedKey = state.leftCheckedKey.filter(n => n !== item.id.toString())
          }
        }
      })
  ...
  function handleLeftChecked(_: string[], {
    ...
    const {eventKey, checked} = node

    itemSelect(eventKey, !isChecked(checkedKeys, eventKey, 'left', state.leftTreeData, checked))
  }
  ...
  function handleRightChecked(_: string[], {
    ...
    const {eventKey, checked} = node
    itemSelect(eventKey, isChecked(_, eventKey, 'right', state.rightTreeData, checked))
  }
