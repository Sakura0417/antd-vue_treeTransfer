 ...
 * @param eventKey
 */
export function isChecked(selectedKeys: string[], eventKey: string, direction: string, data: any, state: boolean): boolean {
  if (direction === 'right') {
...
export function handleLeftTreeData(data: TreeDataItem[], targetKeys: string[], direction = 'right'): TreeDataItem[] {
  ...
  data.forEach(item => {
    if (direction === 'right') {
      ...
      console.log('item===>', info?.children?.some(i => !i.disabled))
